...
usage() {
  cat <<'EOF'
Usage:
  scripts/feishu-claude-manager-quickstart.sh [project-dir]

Default project-dir:
  /Users/aias/Work/github/agent-bot-gateway

What it does:
  1. Verifies `squad` and `claude`
  2. Runs `squad setup claude`
  3. Runs `squad init --refresh-roles` in the target project
  4. Generates:
     - .squad/quickstart/manager-feishu-claude-support.prompt.md
...
project_dir="${1:-/Users/aias/Work/github/agent-bot-gateway}"
...
echo "[1/4] Refreshing Claude /squad command"
squad setup claude

echo "[2/4] Initializing squad workspace"
...
guide_file="$quickstart_dir/terminal-commands.md"
...
echo "[4/4] Writing terminal guide"
cat >"$guide_file" <<EOF
# Quickstart for Feishu + Claude support work
...
echo "Terminal guide: $guide_file"
