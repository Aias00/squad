...
Generated files:
  <project-dir>/.squad/quickstart/generated-manager.prompt.md
  <project-dir>/.squad/quickstart/generated-inspector.prompt.md
  <project-dir>/.squad/quickstart/generated-run-summary.md
  <project-dir>/.squad/quickstart/generated-terminal-map.md
...
build_manager_prompt() {
...

  {
    ...
    echo
    echo "请基于当前项目画像和本次任务说明，组织 squad 多 agent 协作推进工作。"
    echo
...
- 先做只读检查，建立当前能力基线，再拆任务。
- 优先使用 `squad task create / ack / complete` 管理任务状态。
- manager 负责分析、拆解、协调、验收，不要自己承担主要实现。
- 每个任务必须写清楚目标、涉及文件、不可破坏的行为和验收标准。
- 每个 worker 完成后都要交给 inspector 审查。
- 不要只验证 happy path；必须覆盖失败、降级、恢复和回归风险。
- 如果启用了 worktree，所有代码修改、测试和提交都必须在 `Workspace root` 中完成。
...
build_inspector_prompt() {
...

  {
    ...
    echo
    echo "请作为 inspector 审查当前 squad 协作结果，优先关注 bug、回归风险、文档漂移和测试缺口。"
    echo
    echo "## Project Context"
    echo "- Project: \`$project_name\`"
    echo "- Config root: \`$source_project_dir\`"
...
EOF
    ...
    if [[ -f "$inspector_source" ]]; then
      ...
      cat <<'EOF'
## Inspector Brief
请基于本次任务说明审查 manager / worker 的产出，重点确认：
- 实现是否真的满足目标，而不是只让测试变绿
- 是否引入行为回归或兼容性破坏
- README / 配置说明 / 诊断信息是否与实现一致
- 测试是否真正覆盖整改目标
...
if (( worktree_enabled == 1 )); then
  ...
  worktree_location="${worktree_location_override:-${CFG_WORKTREE_LOCATION:-.worktrees}}"
...
echo "[4/5] Sending squad commands"
for i in "${!pane_commands[@]}"; do
  send_tmux_text "$session_name":0."$i" "${pane_commands[$i]}"
done

sleep 3
echo "[5/5] Sending manager and inspector prompts"
